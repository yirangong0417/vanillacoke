function f=focp(price,x1,mc1,nest0,Firms)
%This function calculates firm 5's FOCs when rival firms keep their prices at
%a given level.
...
for j=1:np
    for k=1:np
        if Firms(j)==5 && Firms(k)==5
            omegaf(j,k)=1;
        end
    end
end
sharetemp=share.*omegaf(:,end);
f=norm(sharetemp+omegaf.*lambda*(price-mc1));